...
function upgradeScript() {
    wget -Nq --no-check-certificate -O ./install_kernel.sh "https://raw.githubusercontent.com/cdnf/bbr_kernel/master/bbr_kernel.sh"
    green " Script upgrade successful. 本脚本升级成功! "
    chmod +x ./bbr_kernel.sh && ./bbr_kernel.sh
}
...
function installCentosKernelManual() {
    ...
    if [ "${linuxKernelByUser}" = "elrepo" ]; then
        ...
        if [ "${osReleaseVersionNoShort}" -eq 7 ]; then
            ...
            if [[ "${linuxKernelToInstallVersion}" == "5.10" || "${linuxKernelToInstallVersion}" == "5.15" || "${linuxKernelToInstallVersion}" == "6.1" || "${linuxKernelToInstallVersion}" == "6.6" ]]; then
...
function vps_netflix_jin_auto() {
    ...
    (
        crontab -l
        echo "10 5 * * 0,1,2,3,4,5,6 /root/nf.sh auto >> /root/warp_refresh.log "
    ) | sort - | uniq - | crontab -
    echo
...
function installWARPClient() {
    ...
    (
        crontab -l
        echo "12 6 * * 1,4 systemctl restart warp-svc "
    ) | sort - | uniq - | crontab -
